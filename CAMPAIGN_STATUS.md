...
Completed this run: terminal UI owner-wallet nonce cancel prepare boundary added `nonce-cancel-prepare-trigger.js`, `nonce-cancel-prepare-binding.js`, and `local-api-nonce-cancel-prepare-smoke.test.mjs`; it calls `POST /v1/nonces/cancel`, validates the intentional HTTP `501` `owner_signed_nonce_cancel_not_implemented` envelope, renders `owner-signed-required`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `approvalGate: explicit-approval-required-before-wallet-signing-or-quai-broadcast`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.

Next autonomous slice: testnet cutover readiness Task 6 — terminal UI nonce cancel prepare render panel
