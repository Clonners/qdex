...
test('post-delegate-key owner-signed readiness plan pins completed prepare surfaces and future approval gates', async () => {
  ...
  for (const requiredText of [
    ...
    'read-only DelegateKeyRegistry `DelegateKeyRegistered`/`DelegateKeyRevoked` projection schema ratchet',
  ]) {
...
test('campaign status moves from delegate prepare smoke to the post-delegate readiness checkpoint', async () => {
  ...
  for (const requiredText of [
    ...
    'Completed this run: post-delegate-key owner-signed readiness docs added `docs/plans/2026-06-08-post-delegate-key-owner-signed-readiness.md` plus delegate/core doc links',
    'Next autonomous slice: read-only DelegateKeyRegistry registration/revocation projection schema ratchet',
