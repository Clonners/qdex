...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: Python SDK DelegateKeyRegistry history stream consumers are complete; SDK `dex.delegate_keys.registrations.open_stream()` / `dex.delegate_keys.revocations.open_stream()` plus bounded `delegate_keys.registrations.stream(limit=...)` / `delegate_keys.revocations.stream(limit=...)` helpers consume private `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations` snapshots with `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is another bounded local/source-only MVP surface',
    ...
    'Next autonomous slice: another bounded local/source-only MVP surface; live `DelegateKeyRegistry` mutation remains approval-gated and no wallets/RPC/signing/broadcast/deploy/tx/funds behavior is approved.',
