...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: private DelegateKeyRegistry registration/revocation WebSocket snapshot alignment is complete; `services/api/src/streams.js` and `services/api/test/websocket.test.mjs` expose `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations` snapshots that reuse `delegatekeyregistry-event-projection` history envelopes with `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, empty mock arrays, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI binding for private DelegateKeyRegistry history streams',
    ...
    'Completed this run: private DelegateKeyRegistry registration/revocation WebSocket snapshot alignment added `delegatekeyregistry-event-projection` stream contracts and snapshots for `/v1/ws?channel=delegate-key-registrations` and `/v1/ws?channel=delegate-key-revocations`, reusing `createDelegateKeyHistoryProjectionEnvelope()` for `DelegateKeyRegisteredProjection` / `DelegateKeyRevokedProjection`, empty mock arrays, null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `delegateKeyRegistryMutation: false`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: terminal UI binding for private DelegateKeyRegistry history streams, still preserving `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
