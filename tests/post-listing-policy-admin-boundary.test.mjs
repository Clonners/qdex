...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: TypeScript SDK and qdex CLI public kline/candle consumers are complete; `dex.klines.get(marketId, { interval })`, `dex.klines.openStream(marketId, { interval })`, `dex.klines.stream(marketId, { interval, limit })`, `qdex klines QI-QUAI --interval 1m`, and `qdex stream klines QI-QUAI --interval 1m --limit N` consume public REST/WebSocket candle projections for `/v1/klines/<MARKET>?interval=1m` and `market.<MARKET>.klines.1m` while preserving `kline_snapshot`, `mock-candle-projection`, `public-read-only-no-custody`, bounded local consumption, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is Python SDK public kline/candle consumers',
