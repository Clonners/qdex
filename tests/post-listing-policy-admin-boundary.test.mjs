...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only FeeManager fee schedule API envelope is complete; `GET /v1/fees` now returns local/source-only `feemanager-policy-projection` metadata with `FeeScheduleProjection`, `eventName: FeesUpdated`, `hardMaxFeeBps: 1000`, `feeRecipient: null`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `feeManagerMutation: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is TypeScript/Python/qdex clients for read-only FeeManager fee schedule metadata',
    ...
    'Next autonomous slice: TypeScript/Python/qdex clients for read-only FeeManager fee schedule metadata',
