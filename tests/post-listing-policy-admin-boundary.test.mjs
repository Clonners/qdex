...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only DelegateKeyRegistry registration/revocation projection schema ratchet is complete; `services/indexer/schema.md`, `docs/api-openapi.yaml`, and `docs/delegate-keys.md` now define `DelegateKeyRegisteredProjection` and `DelegateKeyRevokedProjection` event-shaped rows with mock-null evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is read-only delegate-key registration/revocation history API envelopes',
    ...
    'Completed this run: read-only DelegateKeyRegistry registration/revocation projection schema ratchet added `DelegateKeyRegisteredProjection` and `DelegateKeyRevokedProjection` to the indexer schema, OpenAPI, delegate docs, and readiness plan; mock rows keep null tx/block/explorer evidence, real rows require event truth, and every row preserves `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, no live `DelegateKeyRegistry` mutation by projection, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: read-only delegate-key registration/revocation history API envelopes, still preserving `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
