...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: Python SDK public kline/candle consumers are complete; `dex.klines.get(market_id, interval="1m")`, `dex.klines.open_stream(market_id, interval="1m")`, and `dex.klines.stream(market_id, interval="1m", limit=...)` consume public REST/WebSocket candle projections for `/v1/klines/<MARKET>?interval=1m` and `market.<MARKET>.klines.1m` while preserving `kline_snapshot`, `mock-candle-projection`, `public-read-only-no-custody`, bounded local consumption, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI public kline/candle panel binding',
