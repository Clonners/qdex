...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI binding for the FeeManager fee schedule stream is complete; `web/terminal-ui/src/live-fee-policy.js` now consumes public `/v1/ws?channel=fees` snapshots and renders the same `feemanager-policy-projection` / `FeeScheduleProjection` safety envelope with `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `feeManagerMutation: false`, `tradingVaultMutation: false`, no fee-authority runtime keys, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is local API + terminal UI FeeManager fee schedule stream integration smoke',
    ...
    'Next autonomous slice: local API + terminal UI FeeManager fee schedule stream integration smoke',
