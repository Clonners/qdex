...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only TradingVault `Deposit`/`Withdraw` projection schema ratchet is complete; event-shaped deposit/withdrawal projections are pinned in the indexer schema, OpenAPI, and vault docs; next autonomous slice can add read-only vault deposit/withdrawal history API envelopes or another bounded local/source-only MVP surface',
    ...
    'Completed this run: read-only TradingVault `Deposit`/`Withdraw` projection schema ratchet added `TradingVaultDepositProjection` and `TradingVaultWithdrawalProjection` to the indexer schema, OpenAPI, and vault docs; mock rows keep null tx/block/explorer evidence, real rows require event truth, and every row preserves `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: read-only vault deposit/withdrawal history API envelopes backed by the projection schemas or another bounded local/source-only MVP slice; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
