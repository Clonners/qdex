...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI public kline/candle stream integration smoke is complete; `web/terminal-ui/src/kline-stream-binding.js` now reads local `GET /v1/klines/<MARKET>?interval=1m`, subscribes to public `/v1/ws?channel=market.<MARKET>.klines.1m`, and renders the terminal kline stream panel only when REST + WebSocket agree on `source: mock-candle-projection`, `payload: kline_snapshot`, `public-read-only-no-custody`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is another bounded local/source-only MVP surface',
