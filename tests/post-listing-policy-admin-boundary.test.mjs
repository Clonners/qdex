...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only FeeManager fee schedule WebSocket snapshot alignment is complete; `/v1/ws?channel=fees` now publishes public `feemanager-policy-projection` / `FeeScheduleProjection` snapshots reusing `GET /v1/fees` with `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `feeManagerMutation: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds metadata; next autonomous slice is terminal UI binding for the FeeManager fee schedule stream',
    ...
    'Completed this run: read-only FeeManager fee schedule WebSocket snapshot alignment added public `fees` stream contract and `/v1/ws?channel=fees` snapshots',
