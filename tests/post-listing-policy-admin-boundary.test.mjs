...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI vault prepare smoke is complete; the smoke starts local `createApiServer()`, clicks deposit and withdrawal buttons, validates intentional HTTP `501` owner-wallet envelopes, and renders only local/source-only no wallet/RPC/signing/broadcast/deploy/tx/funds metadata; next autonomous slice can add post-vault readiness docs or another bounded local/source-only MVP surface',
    ...
    'Completed this run: local API + terminal UI vault prepare smoke added `src/vault-prepare-binding.js` and a focused browser/API smoke that starts local `createApiServer()`, clicks deposit and withdrawal buttons, validates the intentional HTTP `501` owner-wallet boundary envelopes, and renders only no-wallet/no-RPC/no-signing/no-broadcast/no-deploy/no-tx/no-funds metadata.',
    'Next autonomous slice: post-vault owner-wallet readiness docs or another bounded local/source-only MVP slice; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
