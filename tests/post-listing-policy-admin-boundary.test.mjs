...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI prepare-only owner-wallet TradingVault deposit/withdrawal panel is complete; browser buttons call the existing `501` prepare-only placeholders, render the owner-wallet boundary envelope, and preserve no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice can add a local API + terminal UI vault prepare smoke or another bounded local/source-only MVP surface',
