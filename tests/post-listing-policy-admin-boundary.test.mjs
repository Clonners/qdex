...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only TypeScript/Python/qdex delegate-key history clients are complete; SDK `delegateKeys.listRegistrations()` / `delegateKeys.listRevocations()`, Python `delegate_keys.list_registrations()` / `delegate_keys.list_revocations()`, and `qdex api registrations` / `qdex api revocations` consume `GET /v1/delegate-keys/registrations` and `GET /v1/delegate-keys/revocations` `delegatekeyregistry-event-projection` envelopes with `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI read-only delegate-key history panel',
    ...
    'Completed this run: read-only TypeScript/Python/qdex delegate-key history clients added SDK `delegateKeys.listRegistrations()` / `delegateKeys.listRevocations()`, Python `delegate_keys.list_registrations()` / `delegate_keys.list_revocations()`, and CLI `qdex api registrations` / `qdex api revocations` for `GET /v1/delegate-keys/registrations` and `GET /v1/delegate-keys/revocations`, preserving `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: terminal UI read-only delegate-key history panel for the same projection envelopes, still preserving `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
