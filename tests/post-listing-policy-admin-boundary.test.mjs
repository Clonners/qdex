...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI command-palette skeleton for read-only/local mock actions is complete; `web/terminal-ui/src/command-palette.js` now provides `terminal-command-palette-skeleton` / `local-ui-preview-only` / `preview-only-no-dispatch` metadata, fixture normalization, command previewing, and `bindCommandPaletteSkeleton` for safe local UI previews of `:markets`, `:ticker QI-QUAI`, `:book QI-QUAI`, `:proof trade-000001`, `:balance`, `:account`, `:fees`, `:stream tickers`, `:mock cross`, `:cancel all matcher-local`, `:deposit WQI 10 prepare owner-wallet-only`, and `:api create-key bot-mm-1 prepare NO_WITHDRAW NO_ADMIN`, while preserving `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, `marketRegistryMutation: false`, `delegateKeyRegistryMutation: false`, matcher-local nonce wording, no command dispatch, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is local API + terminal UI command-palette smoke for read-only/local mock actions',
