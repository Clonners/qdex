...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI read-only vault history panel is complete; `src/vault-history-panel.js`, `mockVerticalSliceFixture.vaultHistory`, renderer panel, and README coverage expose local/source-only TradingVault deposit/withdrawal history with `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can add a local API + terminal UI vault history integration smoke or another bounded local/source-only MVP surface',
