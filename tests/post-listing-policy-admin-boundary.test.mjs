...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: qdex public ticker CLI command is complete; `qdex ticker QI-QUAI` now reads `GET /v1/tickers/QI-QUAI` through the existing SDK ticker client, returns `source: mock-market-data` with null mock prices and `volume24h: "0"`, and preserves no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is read-only public market-data stream consumers',
