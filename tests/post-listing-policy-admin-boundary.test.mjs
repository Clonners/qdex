...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only TypeScript SDK and `qdex` CLI vault history stream consumers are complete; `dex.vault.deposits.openStream()` / `dex.vault.withdrawals.openStream()`, bounded `stream({ limit })`, and `qdex stream deposits` / `qdex stream withdrawals` consume private `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals` TradingVault history snapshots while preserving `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, mock-null event evidence, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can mirror the stream consumers in Python SDK or choose another bounded local/source-only MVP surface',
