...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: post-vault owner-wallet readiness docs are complete; they map mock-vault balances and prepare-only deposit/withdrawal surfaces to the explicit owner-wallet approval gate; next autonomous slice can add a read-only TradingVault `Deposit`/`Withdraw` projection schema ratchet or another bounded local/source-only MVP surface',
    ...
    'Completed this run: post-vault owner-wallet readiness docs added `docs/plans/2026-06-08-post-vault-owner-wallet-readiness.md` plus vault/core doc links, mapping mock-vault balance state and prepare-only deposit/withdrawal surfaces to the explicit owner-wallet approval gate before any wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: read-only TradingVault `Deposit`/`Withdraw` projection schema ratchet or another bounded local/source-only MVP slice; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
