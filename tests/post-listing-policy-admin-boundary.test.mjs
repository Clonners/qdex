...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI public market-data stream binding is complete; `web/terminal-ui/src/live-market-data.js` now consumes public `/v1/ws?channel=global.tickers`, `/v1/ws?channel=market.<MARKET>.depth`, and `/v1/ws?channel=market.<MARKET>.trades` snapshots for `ticker_snapshot`, `orderbook_depth`, and `trade_projection`, rendering only `mock-market-data`, `mock-orderbook`, `in-memory-indexer-projection`, `confirmed-settlement-only`, `public-read-only-no-custody`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is local API + terminal UI public market-data stream integration smoke',
