...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: Python SDK public market-data stream consumers are complete; `dex.tickers.open_stream()` / `tickers.stream(limit=...)`, `dex.orderbook.open_stream(market_id)` / `orderbook.stream(market_id, limit=...)`, and `dex.trades.open_stream(market_id)` / `trades.stream(market_id, limit=...)` consume public local WebSocket snapshots for `global.tickers`, `market.<MARKET>.depth`, and `market.<MARKET>.trades` while preserving `ticker_snapshot`, `orderbook_depth`, `trade_projection`, `public-read-only-no-custody`, `mock-market-data`, `mock-orderbook`, `in-memory-indexer-projection`, `confirmed-settlement-only`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is another bounded local/source-only MVP surface',
