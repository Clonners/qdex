...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only vault deposit/withdrawal history API envelopes plus TypeScript/Python/qdex clients are complete; `GET /v1/vault/deposits`, `GET /v1/vault/withdrawals`, SDK `dex.vault.deposits.list()` / `dex.vault.withdrawals.list()`, and CLI `qdex vault deposits` / `qdex vault withdrawals` expose local/source-only `tradingvault-event-projection` metadata with mock-null event evidence; next autonomous slice can add a terminal UI read-only vault history panel or another bounded local/source-only MVP surface',
