...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI vault history stream integration smoke is complete; `web/terminal-ui/src/vault-history-stream-binding.js` first reads `GET /v1/vault/deposits` and `GET /v1/vault/withdrawals`, then subscribes to `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals`, rendering only when REST and WebSocket envelopes agree on `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, mock-null event evidence, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can add read-only SDK/CLI vault history stream consumers or choose another bounded local/source-only MVP surface',
    ...
    'Next autonomous slice: read-only SDK/CLI vault history stream consumers or another bounded local/source-only MVP surface; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
