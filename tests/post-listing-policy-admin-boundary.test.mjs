...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only TypeScript SDK and `qdex` CLI public market-data stream consumers are complete; `dex.tickers.stream({ limit })`, `dex.orderbook.stream(marketId, { limit })`, `dex.trades.stream(marketId, { limit })`, `qdex stream tickers`, `qdex stream depth QI-QUAI`, and `qdex stream trades QI-QUAI` consume public local WebSocket snapshots for `global.tickers`, `market.<MARKET>.depth`, and `market.<MARKET>.trades` while preserving `public-read-only-no-custody` and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is Python SDK public market-data stream consumers',
