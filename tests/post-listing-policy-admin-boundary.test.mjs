...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI public kline/candle panel binding is complete; the static terminal UI fixture now renders read-only `/v1/klines/<MARKET>?interval=1m` style metadata and `bindLiveKlineStream` consumes public `/v1/ws?channel=market.<MARKET>.klines.1m` snapshots while preserving `kline_snapshot`, `mock-candle-projection`, `public-read-only-no-custody`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is local API + terminal UI public kline/candle stream integration smoke',
