...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: Python SDK vault history stream consumers are complete; `dex.vault.deposits.open_stream()` / `dex.vault.withdrawals.open_stream()` and bounded `stream(limit=...)` consume private `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals` TradingVault history snapshots while preserving `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, mock-null event evidence, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can choose another bounded local/source-only MVP surface',
