...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: prepare-only delegate/API key registration/revocation client exposure is complete; `GET /v1/delegate-keys` exposes read-only `delegate-key-registry-projection` metadata, `POST /v1/delegate-keys` and `DELETE /v1/delegate-keys/{keyId}` intentionally return `501` owner-signed placeholder envelopes, and TypeScript/Python SDK plus `qdex api create-key --prepare` / `qdex api revoke-key --prepare` clients now surface those envelopes while preserving `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice can add terminal UI prepare-only delegate-key panel/binding or choose another bounded local/source-only MVP surface',
    ...
    'Completed this run: TypeScript/Python/qdex prepare-only delegate/API key registration and revocation clients added SDK `delegateKeys.prepareRegister()` / `delegateKeys.prepareRevoke()`, Python `delegate_keys.prepare_register()` / `delegate_keys.prepare_revoke()`, and CLI `qdex api create-key --prepare` / `qdex api revoke-key --prepare`; they return intentional `501` owner-signed envelopes with `delegate-key-owner-signed-prepare-boundary`, `prepare-only-owner-signed-required`, `owner-wallet-signature-required`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: terminal UI prepare-only delegate/API key panel/binding or another bounded local/source-only MVP surface; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
