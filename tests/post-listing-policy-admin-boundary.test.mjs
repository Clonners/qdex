...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI private TradingVault `deposits`/`withdrawals` WebSocket binding is complete; `web/terminal-ui/src/live-vault-history.js` consumes `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals`, validates `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`, renders a live vault history stream safety panel while keeping static fallback; next autonomous slice can add a local API + terminal UI vault history stream integration smoke or choose another bounded local/source-only MVP surface',
