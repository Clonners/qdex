...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    'Completed this run: read-only vault deposit/withdrawal history API envelopes added `GET /v1/vault/deposits` and `GET /v1/vault/withdrawals`, backed by `TradingVaultDepositProjection` / `TradingVaultWithdrawalProjection`, `source: tradingvault-event-projection`, `settlementMode: mock`, null `settlementTx`/`blockNumber`/`blockHash`/`eventIndex`/`explorerUrl`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`.',
    'Next autonomous slice: read-only TypeScript/Python/qdex clients for vault deposit/withdrawal history or another bounded local/source-only MVP slice; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
