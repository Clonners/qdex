...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI public market-data stream integration smoke is complete; `web/terminal-ui/src/market-data-stream-binding.js` now checks `GET /v1/tickers`, `GET /v1/orderbook/<MARKET>`, and `GET /v1/trades/<MARKET>` before subscribing to public `/v1/ws?channel=global.tickers`, `/v1/ws?channel=market.<MARKET>.depth`, and `/v1/ws?channel=market.<MARKET>.trades`, rendering only on REST + WebSocket agreement for `ticker_snapshot`, `orderbook_depth`, `trade_projection`, `mock-market-data`, `mock-orderbook`, `in-memory-indexer-projection`, `confirmed-settlement-only`, `public-read-only-no-custody`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI command-palette skeleton for read-only/local mock actions',
