...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only FeeManager fee schedule client exposure is complete; `GET /v1/fees`, TypeScript SDK `fees.get()`, Python SDK `fees.get()`, and `qdex fees` now expose local/source-only `feemanager-policy-projection` metadata with `FeeScheduleProjection`, `eventName: FeesUpdated`, `hardMaxFeeBps: 1000`, `feeRecipient: null`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `feeManagerMutation: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI read-only FeeManager fee schedule exposure',
    ...
    'Completed this run: read-only FeeManager fee schedule clients added TypeScript SDK `fees.get()`, Python SDK `fees.get()`, and `qdex fees` for `GET /v1/fees`, preserving `feemanager-policy-projection`, `FeeScheduleProjection`, `eventName: FeesUpdated`, `hardMaxFeeBps: 1000`, `feeRecipient: null`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `feeManagerMutation: false`, `tradingVaultMutation: false`, no fee-authority runtime keys, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
