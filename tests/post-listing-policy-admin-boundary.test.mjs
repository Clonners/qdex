...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only TypeScript SDK and `qdex` CLI DelegateKeyRegistry history stream consumers are complete; SDK `delegateKeys.registrations.openStream()` / `delegateKeys.revocations.openStream()` plus bounded `stream({ limit })` helpers and CLI `qdex stream delegate-key-registrations` / `qdex stream delegate-key-revocations` consume private `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations` snapshots with `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is Python SDK DelegateKeyRegistry history stream consumers',
