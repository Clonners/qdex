...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI private DelegateKeyRegistry history stream binding is complete; `web/terminal-ui/src/live-delegate-key-history.js`, `render.js`, `app.js`, README/docs, package checks, and `live-delegate-key-history.test.mjs` bind `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations` snapshots into the read-only delegate/API key history panel with `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is a local API + terminal UI DelegateKeyRegistry history stream integration smoke that proves REST + WebSocket agreement before rendering',
    ...
    'Completed this run: terminal UI private DelegateKeyRegistry history stream binding added `src/live-delegate-key-history.js`, renderer/app/README/package coverage, and optional `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations` consumers that validate `delegatekeyregistry-event-projection` envelopes before rendering the read-only delegate/API key history panel with `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, mock-null event evidence, no live `DelegateKeyRegistry` mutation, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: local API + terminal UI DelegateKeyRegistry history stream integration smoke, still preserving REST + WebSocket agreement on `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
