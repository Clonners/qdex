...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only delegate-key registration/revocation history API envelopes are complete; `GET /v1/delegate-keys/registrations` and `GET /v1/delegate-keys/revocations` now expose empty/mock `delegatekeyregistry-event-projection` envelopes with `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, no live `DelegateKeyRegistry` mutation, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is read-only TypeScript/Python/qdex delegate-key history clients',
    ...
    'Completed this run: read-only delegate-key registration/revocation history API envelopes added `GET /v1/delegate-keys/registrations` and `GET /v1/delegate-keys/revocations`, backed by `DelegateKeyRegisteredProjection` / `DelegateKeyRevokedProjection`, `source: delegatekeyregistry-event-projection`, `settlementMode: mock`, null `settlementTx`/`blockNumber`/`blockHash`/`eventIndex`/`explorerUrl`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: read-only TypeScript/Python/qdex delegate-key history clients for `GET /v1/delegate-keys/registrations` and `GET /v1/delegate-keys/revocations`, still preserving `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
