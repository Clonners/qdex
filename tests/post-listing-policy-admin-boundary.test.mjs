...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI vault history integration smoke is complete; `src/vault-history-binding.js` and `local-api-vault-history-smoke.test.mjs` start local `createApiServer()`, read `GET /v1/vault/deposits` plus `GET /v1/vault/withdrawals`, feed both REST envelopes through the terminal UI normalizer/renderer, and prove REST + panel agree on `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can align private `deposits`/`withdrawals` WebSocket snapshots with the same TradingVault event-projection envelope or choose another bounded local/source-only MVP surface',
