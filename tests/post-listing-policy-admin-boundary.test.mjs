...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: read-only account overview clients are complete; TypeScript SDK `account.get()`, Python SDK `account.get()`, and `qdex account` now call `GET /v1/account` and preserve local/source-only `mock-account-overview` metadata that composes `mock-vault-projection` balances, matcher-local `mock-order-projection` open orders, and confirmed-only `IndexedFillProjection` rows with `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is terminal UI read-only account overview panel',
