...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: prepare-only owner-wallet TradingVault deposit/withdrawal client exposure is complete across API, OpenAPI, docs, TypeScript/Python SDKs, `qdex` CLI, and ratchets; next autonomous slice can bind a terminal UI prepare-only vault operation panel or another bounded local/source-only MVP surface; no wallets/RPC/deploys/txs/funds are approved',
