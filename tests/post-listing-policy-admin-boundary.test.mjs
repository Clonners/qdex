...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI DelegateKeyRegistry history stream integration smoke is complete; `web/terminal-ui/src/delegate-key-history-stream-binding.js` and `local-api-delegate-key-history-stream-smoke.test.mjs` start local `createApiServer()`, read `GET /v1/delegate-keys/registrations` plus `GET /v1/delegate-keys/revocations`, subscribe to `/v1/ws?channel=delegate-key-registrations` plus `/v1/ws?channel=delegate-key-revocations`, and render only when REST + WebSocket agree on `delegatekeyregistry-event-projection`, `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is read-only TypeScript SDK and `qdex` CLI DelegateKeyRegistry history stream consumers',
