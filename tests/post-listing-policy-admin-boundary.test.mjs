...
test('campaign status records completion-mode continuation plus local listing and balance projection checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI balance projection binding is complete across the private `balances` WebSocket consumer, renderer panel, browser app binding, README, and ratchets; next autonomous slice should be a local API + terminal UI balances stream integration smoke or another bounded local/source-only MVP slice with no real network mutation; no wallets/RPC/deploys/txs are approved',
    ...
    'Completed this run: terminal UI balance projection binding added a private `balances` WebSocket consumer, mock-vault renderer panel, browser app binding, README docs, and ratchets while preserving `mock-vault-projection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, and no wallet/funds behavior.',
    'Next autonomous slice: local API + terminal UI balances stream integration smoke or another bounded local/source-only MVP slice without listing submission, real token addresses, wallets/RPC/signing/broadcast/deploy/tx behavior, funds movement, or real network `MarketRegistry` mutation.',
