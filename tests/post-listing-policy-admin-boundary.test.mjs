...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: terminal UI prepare-only delegate/API key panel/binding is complete; `src/delegate-key-prepare-trigger.js` and renderer/app wiring call existing `POST /v1/delegate-keys` / `DELETE /v1/delegate-keys/{keyId}` owner-signed `501` boundaries from local UI buttons, render the display-only envelope, and preserve `owner-wallet-signature-required`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, no live `DelegateKeyRegistry` mutation, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice can add a local API + terminal UI delegate/API key prepare smoke or choose another bounded local/source-only MVP surface',
    ...
    'Completed this run: terminal UI prepare-only delegate/API key panel/binding added `src/delegate-key-prepare-trigger.js`, browser buttons, renderer panel, app wiring, README coverage, and ratchets for `POST /v1/delegate-keys` plus `DELETE /v1/delegate-keys/{keyId}` owner-signed `501` envelopes; it renders `delegate-key-owner-signed-prepare-boundary`, `prepare-only-owner-signed-required`, `owner-wallet-signature-required`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, no live `DelegateKeyRegistry` mutation, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: local API + terminal UI delegate/API key prepare smoke or another bounded local/source-only MVP surface; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
