...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI delegate-key history integration smoke is complete; `src/delegate-key-history-binding.js` and `local-api-delegate-key-history-smoke.test.mjs` start local `createApiServer()`, read `GET /v1/delegate-keys/registrations` plus `GET /v1/delegate-keys/revocations`, normalize both `delegatekeyregistry-event-projection` envelopes, render the read-only delegate/API key history panel with `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, empty mock arrays, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice is private DelegateKeyRegistry registration/revocation WebSocket snapshot alignment',
    ...
    'Completed this run: local API + terminal UI delegate-key history integration smoke added `src/delegate-key-history-binding.js` and `local-api-delegate-key-history-smoke.test.mjs`; it starts local `createApiServer()`, reads `GET /v1/delegate-keys/registrations` plus `GET /v1/delegate-keys/revocations`, feeds both `delegatekeyregistry-event-projection` envelopes through the terminal UI normalizer/renderer, treats empty mock arrays as valid state, and preserves `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: private DelegateKeyRegistry registration/revocation WebSocket snapshot alignment for `/v1/ws` channels carrying the same `delegatekeyregistry-event-projection` history envelopes, still preserving `DelegateKeyRegisteredProjection`, `DelegateKeyRevokedProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `delegateKeyRegistryMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
