...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: private TradingVault `deposits`/`withdrawals` WebSocket snapshot alignment is complete; `services/api/src/streams.js` and API WebSocket transport reuse `createVaultHistoryProjectionEnvelope()` for `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals`, carrying `source: tradingvault-event-projection`, `TradingVaultDepositProjection`, `TradingVaultWithdrawalProjection`, mock-null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`; next autonomous slice can bind terminal UI to those vault history streams or choose another bounded local/source-only MVP surface',
    ...
    'Completed this run: private `deposits`/`withdrawals` WebSocket snapshot alignment added `tradingvault-event-projection` stream contracts and snapshots for `/v1/ws?channel=deposits` and `/v1/ws?channel=withdrawals`, reusing `createVaultHistoryProjectionEnvelope()` for `TradingVaultDepositProjection` / `TradingVaultWithdrawalProjection`, empty mock arrays, null event evidence, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`.',
    'Next autonomous slice: bind terminal UI to private `deposits`/`withdrawals` vault history streams or choose another bounded local/source-only MVP surface; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
