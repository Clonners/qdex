...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: prepare-only delegate/API key registration and revocation API boundary is complete; `GET /v1/delegate-keys` exposes read-only `delegate-key-registry-projection` metadata, while `POST /v1/delegate-keys` and `DELETE /v1/delegate-keys/{keyId}` intentionally return `501` owner-signed placeholder envelopes preserving `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior; next autonomous slice can expose prepare-only delegate-key clients or choose another bounded local/source-only MVP surface',
    ...
    'Completed this run: prepare-only delegate/API key registration and revocation API boundary added `delegate-key-registry-projection` list metadata plus intentional `501` owner-signed placeholders for `POST /v1/delegate-keys` and `DELETE /v1/delegate-keys/{keyId}`; responses preserve `NO_WITHDRAW`, `NO_ADMIN`, `delegateCanWithdraw: false`, `delegateCanAdmin: false`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, `tradingVaultMutation: false`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.',
    'Next autonomous slice: TypeScript/Python/qdex prepare-only delegate-key registration/revocation clients or another bounded local/source-only MVP surface; still no wallets/RPC/signing/broadcast/deploy/tx/funds behavior.',
