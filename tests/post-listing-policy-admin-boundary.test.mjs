...
test('campaign status records completion-mode continuation plus local listing, balance, and vault prepare checkpoints', async () => {
  ...
  for (const requiredText of [
    ...
    '- Current phase: local API + terminal UI FeeManager fee schedule integration smoke is complete; `web/terminal-ui/src/fee-policy-binding.js` now reads local `GET /v1/fees`, validates the same read-only `feemanager-policy-projection` / `FeeScheduleProjection` safety envelope, and renders only no-wallet/no-RPC/no-signing/no-broadcast/no-deploy/no-tx/no-funds metadata; next autonomous slice is read-only FeeManager fee schedule WebSocket snapshot alignment',
    ...
    'Completed this run: local API + terminal UI FeeManager fee schedule integration smoke added `web/terminal-ui/src/fee-policy-binding.js` and `local-api-fee-policy-smoke.test.mjs`',
