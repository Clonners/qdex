...
- `index.html` renders a static terminal-native trade/proof panel, then attempts local `fills`, `orders`, and `balances` WebSocket bindings at `http://127.0.0.1:8787`, plus prepare-only vault operation triggers and read-only vault history REST smoke.
