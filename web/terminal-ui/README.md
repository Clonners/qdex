...
- `index.html` renders a static terminal-native trade/proof panel, then attempts local `fills`, `orders`, `balances`, `deposits`, `withdrawals`, `delegate-key-registrations`, and `delegate-key-revocations` WebSocket bindings at `http://127.0.0.1:8787`, plus prepare-only vault and delegate/API key operation triggers, read-only vault history REST smoke, read-only delegate-key history REST smoke, and a REST-confirmed vault history stream smoke.
