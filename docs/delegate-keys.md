...
Completed local/source-only SDK/CLI stream consumers: TypeScript SDK `delegateKeys.registrations.openStream()` / `delegateKeys.revocations.openStream()` plus bounded `stream({ limit })`, and CLI `qdex stream delegate-key-registrations` / `qdex stream delegate-key-revocations`, now consume the private read-only `delegatekeyregistry-event-projection` snapshots without live `DelegateKeyRegistry` mutation and without wallet/RPC/signing/broadcast/deploy/tx/funds behavior.

Next local/source-only step: Python SDK DelegateKeyRegistry history stream consumers, still with no live `DelegateKeyRegistry` mutation and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.
