...
open_orders = dex.account.orders()  # GET /v1/account/orders -> mock-order-projection, LocalOrderProjection, READ_ONLY, NO_WITHDRAW, NO_ADMIN, matcherLocalOnly, mock-settlement-only
...
orders_stream = dex.orders.open_stream()  # /v1/ws?channel=orders
order_snapshot = orders_stream.next()
orders_stream.close()
order_stream_snapshots = dex.orders.stream(limit=limit)
...
`klines.get(market_id, interval="1m")` reads public candle projections from `/v1/klines/<MARKET>?interval=1m`; `klines.open_stream(market_id, interval="1m")` and `klines.stream(market_id, interval="1m", limit=limit)` consume bounded public candle snapshots from `/v1/ws?channel=market.<MARKET>.klines.1m`. Kline messages preserve `kline_snapshot`, `public-read-only-no-custody`, `mock-candle-projection`, local mock candle rows, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior. A standalone test `test_python_sdk_consumes_public_klines_stream_without_custody_authority` covers the full `open_stream`/`stream` bounded pattern with 17 assertions.

`tickers.open_stream()` and bounded `tickers.stream(limit=limit)` consume public ticker snapshots from `/v1/ws?channel=global.tickers`. These snapshots preserve `ticker_snapshot`, `public-read-only-no-custody`, `mock-market-data`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.

`orderbook.open_stream(market_id)` and bounded `orderbook.stream(market_id, limit=limit)` consume public depth snapshots from `/v1/ws?channel=market.<MARKET>.depth`. These snapshots preserve `orderbook_depth`, `public-read-only-no-custody`, `mock-orderbook`, `data.marketId`, `data.bids` (empty mock array), `data.asks` (empty mock array), and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior. A standalone test `test_python_sdk_consumes_public_orderbook_depth_stream_without_custody_authority` covers the full `open_stream`/`stream` bounded pattern with 22 assertions.

`trades.open_stream(market_id)` and bounded `trades.stream(market_id, limit=limit)` consume public trade projections from `/v1/ws?channel=market.<MARKET>.trades`. These snapshots preserve `trade_projection`, `public-read-only-no-custody`, `in-memory-indexer-projection`, `confirmed-settlement-only` semantics, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.
...
`account.orders()` is a read-only local open orders projection from `GET /v1/account/orders`. It returns `source: mock-order-projection`, `projectionType: LocalOrderProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `matcherLocalOnly: true`, `settlementMode: mock`, `realQuaiTransactions: False`, `walletRequired: False`, `fundsMoved: False`, and `tradingVaultMutation: False` with mock-null tx/block/event/explorer evidence; it has no wallet/RPC/signing/broadcast/deploy/tx/funds behavior and cannot grant delegate withdrawal/admin authority.

`account.orders.open_stream()` and bounded `account.orders.stream(limit=limit)` consume private open orders snapshots from `/v1/ws?channel=open-orders`. Stream snapshots preserve `source: mock-order-projection`, `payload: open_orders_projection`, `custody: non-custodial-no-withdrawal-authority`, `projectionType: LocalOrderProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `matcherLocalOnly: True`, `settlementMode: mock`, `realQuaiTransactions: False`, `walletRequired: False`, `fundsMoved: False`, and `tradingVaultMutation: False`; there is no wallet/RPC/signing/broadcast/deploy/tx/funds behavior. A standalone test `test_python_sdk_consumes_private_open_orders_stream_without_wallet_authority` covers the full `open_stream`/`stream` bounded pattern with 36 assertions.
...
`orders.open_stream()` and bounded `orders.stream(limit=limit)` consume private order projection snapshots from `/v1/ws?channel=orders`. Stream snapshots preserve `source: mock-order-projection`, `payload: order_projection`, `custody: non-custodial-no-withdrawal-authority`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `matcher-local-cancel-only-on-chain-nonce-unchanged`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.

`trades.open_stream(market_id)` and bounded `trades.stream(market_id, limit=limit)` consume public trade projection snapshots from `/v1/ws?channel=market.<MARKET>.trades`. Stream snapshots preserve `source: in-memory-indexer-projection`, `payload: trade_projection`, `visibility: public`, `custody: public-read-only-no-custody`, `data.marketId`, `data.trades` (empty mock array), `data.source: in-memory-indexer-projection`, and `confirmed-settlement-only` semantics; there is no wallet/RPC/signing/broadcast/deploy/tx/funds behavior. A standalone test `test_python_sdk_consumes_public_trades_stream_without_custody_authority` covers the full `open_stream`/`stream` tickers pattern.

`tickers.open_stream()` and bounded `tickers.stream(limit=limit)` consume public ticker projection snapshots from `/v1/ws?channel=global.tickers`. Stream snapshots preserve `source: mock-market-data`, `payload: ticker_snapshot`, `visibility: public`, `custody: public-read-only-no-custody`, `data.tickers[0].marketId`, `data.tickers[0].source`, `data.tickers[0].volume24h`, `data.tickers[0].lastPrice` (null mock), `data.tickers[0].bestBid` (null mock), `data.tickers[0].bestAsk` (null mock), and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior. A standalone test `test_python_sdk_consumes_public_tickers_stream_without_custody_authority` covers the full `open_stream`/`stream` bounded pattern with 18 assertions.
