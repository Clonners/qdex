...
`dex.tickers.openStream` and `dex.tickers.stream` consume public ticker snapshots from `/v1/ws?channel=global.tickers`; messages carry `ticker_snapshot`, `public-read-only-no-custody`, and `mock-market-data`. A standalone test (`TypeScript SDK consumes public tickers stream without custody authority`) validates `openStream` snapshot structure and bounded `stream({ limit })` against the local API, asserting channel `global.tickers`, visibility `public`, payload `ticker_snapshot`, source `mock-market-data`, custody `public-read-only-no-custody`, and mock-null `lastPrice`/`bestBid`/`bestAsk`. `dex.orderbook.openStream` and `dex.orderbook.stream` consume `/v1/ws?channel=market.<MARKET>.depth`; messages carry `orderbook_depth`, `public-read-only-no-custody`, and `mock-orderbook`. A standalone test validates both `openStream` snapshot structure and bounded `stream({ limit })` against the local API, asserting channel `market.WQUAI-WQI.depth`, visibility `public`, payload `orderbook_depth`, source `mock-orderbook`, custody `public-read-only-no-custody`, and mock-empty `bids`/`asks`. `dex.trades.openStream` and `dex.trades.stream` consume `/v1/ws?channel=market.<MARKET>.trades`; messages carry `trade_projection`, `public-read-only-no-custody`, and `in-memory-indexer-projection` / `confirmed-settlement-only` trade projection semantics. A standalone test validates `openStream` snapshot structure and bounded `stream({ limit })` against the local API, asserting channel `market.WQUAI-WQI.trades`, visibility `public`, payload `trade_projection`, source `in-memory-indexer-projection`, custody `public-read-only-no-custody`, and mock-empty `trades`. These public market-data helpers preserve no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.
...
`dex.account.orders.get()` calls `GET /v1/account/orders` and returns the read-only `mock-order-projection` envelope with `projectionType: LocalOrderProjection`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `matcherLocalOnly: true`, `settlementMode: mock`, mock-null `settlementTx`/`blockNumber`/`blockHash`/`eventIndex`/`explorerUrl`, `realQuaiTransactions: false`, `walletRequired: false`, `fundsMoved: false`, and `tradingVaultMutation: false`. It has no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.

`dex.account.orders.openStream()` and `dex.account.orders.stream({ limit })` consume private open orders snapshots from `/v1/ws?channel=open-orders`. Stream snapshots preserve `open_orders_projection`, `mock-order-projection`, `LocalOrderProjection`, `non-custodial-no-withdrawal-authority`, `matcherLocalOnly: true`, `READ_ONLY`, `NO_WITHDRAW`, `NO_ADMIN`, `settlementMode: mock`, and no wallet/RPC/signing/broadcast/deploy/tx/funds behavior.
